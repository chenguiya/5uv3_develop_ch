...
// 	$navigation = ' <em>&rsaquo;</em> <a href="group.php">'.$_G['setting']['navs'][3]['navname'].'</a> '.$nav['nav'];
	...
	// zhangjh 2015-06-25 upnavlink
	// $upnavlink = 'forum.php?mod=forumdisplay&amp;fid='.$_G['fid'].($_GET['extra'] && !IS_ROBOT ? '&amp;'.$_GET['extra'] : '');
	$upnavlink = 'group/'.$_G['fid'].'/'.($_GET['extra'] && !IS_ROBOT ? '&amp;'.$_GET['extra'] : '');
	...
	if($_G['forum']['type'] == 'sub') {
		...
		$t_link = $_G['cache']['forums'][$fup]['type'] == 'group' ? 'group/'.$fup.'/' : 'group/'.$fup.'/';
		// $navigation .= ' <em>&rsaquo;</em> <a href="'.$t_link.'" target="_blank">'.($_G['cache']['forums'][$fup]['name']).'</a>'; 2015-07-08 zhangjh 不显示上级的联赛
	}
...
if($_G['forum']['password'] && $_G['forum']['password'] != $_G['cookie']['fidpw'.$_G['fid']]) {
	...
	dheader("Location: $_G[siteurl]group/$_G[fid]/");
}
...
if(empty($_GET['viewpid'])) {
	...
	$multipage = multi($_G['forum_thread']['replies'] + 1, $_G['ppp'], $page, 'forum.php?mod=viewthread&tid='.$_G['tid'].
		...
		$specialextra);
	*/
	// 2015-06-30 zhangjh 精简这个分页输出
// 	include_once(DISCUZ_ROOT.'./source/plugin/fansclub/function.inc.php');
	$multipage = fansclub_multi($_G['forum_thread']['replies']+1, $_G['ppp'], $page, 'thread-'.$_G['tid'], '-', '.html');
...
if(empty($_GET['viewpid'])) {
	...
	// zhangjh 2015-06-26 检查是否是马甲发的
	set_mask_in_thread($_G['tid']);
...
function viewthread_procpost($post, $lastvisit, $ordertype, $maxposition = 0) {
	...
	// zhangjh 2015-07-10 可以支持 <embed type="application/x-shockwave-flash"  发这种视频
	$arr_type = C::t('forum_threadclass')->fetch($_G['forum_thread']['typeid']);
	if($arr_type['name'] == '视频' && strpos($org_message, '<embed type="application/x-shockwave-flash"') !== FALSE)
	{
		$post['message'] = $org_message;
	}
