...
// 	var_dump($applylist);die;	
	...
	if ($_GET['mobile']) {
// 		if ($_GET['ajax'] == 1) {
// 			$maxpage = @ceil($activity['applynumber']/$pp);
// 			$nextpage = ($page + 1) > $maxpage ? 1 : ($page + 1);
// 			echo json_encode(array('nextpage' => $nextpage, 'applylist' => $applylist));
// 			exit;
// 		}
		$maxpage = @ceil($activity['applynumber']/$pp);
		$multi = multi($activity['applynumber'], $pp, $page, "forum.php?mod=misc&action=getactivityapplylist&tid=$_G[tid]");
		include template('touch/forum/activity_applylist');
	} else {
...
} elseif ($_GET['action'] == 'activityunauditedlist') {
	$pp = $_G['setting']['activitypp'];
	$page = max(1, $_G['page']);
	$start = ($page - 1) * $pp;
	$activity = C::t('forum_activity')->fetch($_G['tid']);
	...
	$query = C::t('forum_activityapply')->fetch_all_unaudited_for_thread($_G['tid'], $start, $pp);
	foreach($query as $activityapplies) {
		$activityapplies['dateline'] = dgmdate($activityapplies['dateline']);
		$applylist[] = $activityapplies;
	}
	if ($_GET['wap'] == 1 || $_GET['mobile']) {
		if ($_GET['ajax'] == 1) {
			;
		}
		include template('touch/forum/activity_applylist');
	...
	if(!submitcheck('applylistsubmit')) {
		...
		$query = C::t('forum_activityapply')->fetch_all_unaudited_for_thread($_G['tid'], $start, $pp, $_GET['uid'], $isactivitymaster);
		foreach($query as $activityapplies) {
			$ufielddata = '';
			$activityapplies['dateline'] = dgmdate($activityapplies['dateline'], 'u');
			$activityapplies['ufielddatanew'] = dunserialize($activityapplies['ufielddata']);
		...
		$count = C::t('forum_activityapply')->count_apply_by_tid($_G['tid'], 0);
		$maxpage = @ceil($count/$pp);
		$multi = multi($count, $pp, $page, "forum.php?mod=misc&action=activityapplylist&tid=$_G[tid]");
...
// 		var_dump($multi);die;
		...
		if(empty($_GET['applyidarray'])) {
            ...
            $_GET['operation'] = ($_GET['objkey'] == 'applylistsubmit_delete') ? 'delete' : $_GET['operation'];
	...
	$tid = intval($_GET['tid']);
	$activity = $activity_fields = array();
	$activity = DB::fetch_first("SELECT * FROM ".DB::table('forum_thread')." WHERE tid=".$tid);
...
// 	var_dump($userlists);die;
	...
	$multi = multi($count, $pp, $page, "forum.php?mod=misc&action=signinlist&tid=$_GET[tid]&issigined=$issigned");
	include template('extend/forum/activity_signlist');
}
