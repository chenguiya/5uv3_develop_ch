 ...
 *
 *      $Id: weibo.class.php 29558 2013-04-30 22:09:39Z mpage $
 */
...
class plugin_mpage_weibo {
	
	function plugin_mpage_weibo() {
		global $_G;

		$this->weibo_uid = $_G['cache']['plugin']['mpage_weibo']['weibo_uid'];
		$this->weibo_position = $_G['cache']['plugin']['mpage_weibo']['weibo_position'];
	}

	function global_login_extra(){
		global $_G;

		include template('mpage_weibo:global_login_extra');
		return $return;
	}
	...
	function global_usernav_extra1(){
		...
		if($_G['uid'] && !$bind = C::t('#mpage_weibo#mpage_weibo')->fetch($_G['uid'])) {
			include template('mpage_weibo:global_usernav_extra1');
			return $return;
		}
	}
...
class plugin_mpage_weibo_forum extends plugin_mpage_weibo {
...
class plugin_mpage_weibo_member extends plugin_mpage_weibo {

	function logging_top() {
		global $_G;

		if($_G['cookie']['weibo_token']) {
			include template('mpage_weibo:logging_top');
			return $return;
		}
	}
	...
	function register_top() {
		...
		if($_G['cookie']['weibo_token']) {
			...
			include template('mpage_weibo:register_top');
			return $return;
		}
	}
	...
	function register() {
		...
		$_G['setting']['reglinkname'] = lang('plugin/mpage_weibo', 'register_profile');
	}
