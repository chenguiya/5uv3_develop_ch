	...
	if($attachnew) {
		...
		foreach($attachnew as $aid => $attach) {
			...
			if(!empty($newattachfile[$aid])) {
				...
				// git need other handle by zhangjh 2015-07-28 
				$getimagesize = getimagesize($_G['setting']['attachdir'].'/forum/'.$newattachfile[$aid]);
				if(strpos($getimagesize['mime'], 'gif'))
				{
					$is_gif = TRUE;
				}
				else
				...
				if($_G['setting']['watermarkstatus'] && empty($_G['forum']['disablewatermark']) && !$is_gif) {
				//if($_G['setting']['watermarkstatus'] && empty($_G['forum']['disablewatermark'])) { // 可以支持gif水印
