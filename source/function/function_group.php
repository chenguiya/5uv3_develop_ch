...
function groupperm(&$forum, $uid, $action = '', $isgroupuser = '') {
	...
	// if(is_array($isgroupuser['level']) && $isgroupuser['level'] === 0) { // zhangjh 2015-6-7 这个判断好像错了
	if(is_array($isgroupuser) && $isgroupuser['level'] === '0') { // 应该是这样子
		return 5;
	}
...
// 			$groupthreadlist[$thread['tid']]['dateline'] = dgmdate($thread['dateline'], 'd');
			$groupthreadlist[$thread['tid']]['dateline'] = date('Y-m-d H:i', $thread['dateline']);
			...
			$groupthreadlist[$thread['tid']]['recommend_add'] = $thread['recommend_add'];
...
function update_groupmoderators($fid) {
	...
	// 2015-05-15 zhangjh 1,2,6 为管理员level
	$moderators = C::t('forum_groupuser')->groupuserlist($fid, 'level_join', 0, 0, array('level' => array('1', '2', '6')), array('username', 'level'));
	if(!empty($moderators)) {
