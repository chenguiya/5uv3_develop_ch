...
// add by zhangjh 2015-09-25 for mobile version
function getforumimg2($aid, $nocache = 0, $w = 140, $h = 140, $type = '')
{
    global $_G;
    $key = dsign($aid.'|'.$w.'|'.$h);
    $url0 = 'forum.php?mod=image&aid='.$aid.'&size='.$w.'x'.$h.'&key='.rawurlencode($key).($nocache ? '&nocache=yes' : '').($type ? '&type='.$type : '');
    $url = $_G['siteurl'].$url0;
    
    //$url = 'http://www.5usport.com/forum.php?mod=image&aid=12571&size=600x600&key=2c1be3e499381b43&type=fixnone';
    $jump = http_header($url);
