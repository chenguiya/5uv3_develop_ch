...
EOT;
	...
	if(!empty($_GET['fidarray'])) {
		...
		if(submitcheck('validate')) {
			...
			$updateforum = '-';
		} elseif(submitcheck('not_validate')) { // zhangjh 2015-05-25 多增加球迷会审核不通过 和 查看球迷会申请页面
			$updateforum = '';
			$no_pass = TRUE;
		}
		...
		{
			// zhangjh 2015-05-25 审核不通过的相关处理 todo 还没有处理
			cpmsg('群组审核不通过成功', 'action=group&operation=mod', 'succeed');
		}
		
	}elseif(!empty($_GET['fidarray2'])) // zhangjh 2015-06-04 增加5u认证和机构认证的处理和显示
	{
		$groups2 = array();
		if(submitcheck('validate2')) // 点了“通过”审核球迷会
		{
			$apply_ids = $_GET['fidarray2'];
			$confirm_uid = $_G['uid'];
			$confirm_time = TIMESTAMP;
			$confirm_remark = '通过';
			C::t('#fansclub#plugin_fansclub_level_apply_log')->validate_level_for_group($apply_ids, $confirm_uid, $confirm_time, $confirm_remark);
			
			foreach($apply_ids as $key => $value) 
			{
				// 根据apply_id要申请人ID，然后发通知站内信息
				$arr_apply = C::t('#fansclub#plugin_fansclub_level_apply_log')->fetch($value);
				if(count($arr_apply) > 0)
				{
					$config = include DISCUZ_ROOT.'./source/plugin/fansclub/data/config.php';
					$group_info = C::t('forum_forum')->fetch($arr_apply['fid']);
					$forum_info = C::t('forum_forum')->fetch($arr_apply['relation_fid']);
					notification_add($arr_apply['uid'], 'group', '恭喜您，您对【'.$forum_info['name'].'】下的球迷会【'.$group_info['name'].'】提交的申请【'.$config['group_level_special'][$arr_apply['level_type']].'】，审核成功', array(), 1);
				}
			}
			cpmsg('球迷会审核成功', 'action=group&operation=mod', 'succeed');
		}
		elseif(submitcheck('not_validate2'))
		{
			$apply_ids = $_GET['fidarray2'];
			$confirm_uid = $_G['uid'];
			$confirm_time = TIMESTAMP;
			$confirm_remark = '不通过';
			C::t('#fansclub#plugin_fansclub_level_apply_log')->not_validate_level_for_group($apply_ids, $confirm_uid, $confirm_time, $confirm_remark);
			...
			foreach($apply_ids as $key => $value) 
			{
				...
				if(count($arr_apply) > 0)
				{
					...
					$group_info = C::t('forum_forum')->fetch($arr_apply['fid']);
					$forum_info = C::t('forum_forum')->fetch($arr_apply['relation_fid']);
					notification_add($arr_apply['uid'], 'group', '很遗憾，您对【'.$forum_info['name'].'】下的球迷会【'.$group_info['name'].'】提交的申请【'.$config['group_level_special'][$arr_apply['level_type']].'】，审核不成功', array(), 1);
				}
			}
			cpmsg('球迷会审核成功', 'action=group&operation=mod', 'succeed');
		}
	...
	// zhangjh 2015-05-25 多显示申请页面
	foreach($query as $group) {
		$apply = C::t('#fansclub#plugin_fansclub_apply_log')->fetch_all_by_fid($group['fid']);
	...
	echo "<hr>";
	$perpage2 = 50;
	$page2 = intval($_GET['page2']) ? intval($_GET['page2']) : 1;
	$startlimit2 = ($page2 - 1) * $perpage2;
	$count2 = C::t('#fansclub#plugin_fansclub_level_apply_log')->validate_level_num();
	$multipage2 = multi($count2, $perpage2, $page2, ADMINSCRIPT."?action=group&operation=mod&submit=yes");
	$query2 = C::t('#fansclub#plugin_fansclub_level_apply_log')->fetch_all_validate($startlimit2, $startlimit2+$perpage2);
	$config = include DISCUZ_ROOT.'./source/plugin/fansclub/data/config.php';
	foreach($query2 as $group2)
	{
		$groups2 .= showtablerow('', array('class="td25"', '', ''), array(
			"<input type=\"checkbox\" name=\"fidarray2[]\" value=\"$group2[apply_id]\" class=\"checkbox\">",
			"<a href=\"forum.php?mod=forumdisplay&fid=$group2[fid]\" target=\"_blank\">$group2[c_name]</a>",
			$group2['f_name'],
			"<a href=\"home.php?mod=space&uid=$group2[uid]\" target=\"_blank\">$group2[username]</a>",
			dgmdate($group2['log_time']),
			$config['group_level_special'][$group2['level_type']],
			($group2['verify_pic'] == '' ? '-' : '<a title="点击查看" target="_blank" href="'.$_G['siteurl'].'data/attachment/group/'.$group2['verify_pic'].'"a>点击查看</a>'),
			($group2['active_month'] == 0 ? '-' :  $group2['active_month'].'个月'),
			($group2['active_month'] == 0 ? '-' :  dgmdate($group2['expired_time']))
			
		), TRUE);
	}
	
	shownav('group', 'nav_group_mod');
	showsubmenu('5U或机构认证审核');
	showformheader("group&operation=mod");
	showtableheader('待审核球迷会');
	showsubtitle(array('', '球迷会名称', '球迷会版块', '申请者', '申请时间', '申请项目', '提交图片', '有效期', '过期时间'));
	echo $groups2;
	showsubmit('', '', '', '<input type="checkbox" name="chkall" id="chkall" class="checkbox" onclick="checkAll(\'prefix\', this.form, \'fidarray2\')" /><label for="chkall">'.cplang('select_all').'</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="btn" name="validate2" value="通过" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="btn" name="not_validate2" value="不通过" />', $multipage2);
	showtablefooter();
	showformfooter();
	// =============== 5u认证和机构认证显示 结束 ===============
}
