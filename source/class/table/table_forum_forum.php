...
class table_forum_forum extends discuz_table
{
	...
	public function fetch_all_name_by_fid($fids) {
		...
		return DB::fetch_all('SELECT fid, name FROM '.DB::table($this->_table)." WHERE ".DB::field('fid', $fids), array(), 'fid');
                       ...
                       //add by xurui 2015-09-21
        	public function my_fetch_all_name_by_fid($fids) {
		if(empty($fids)) {
			return array();
		}
		return DB::fetch_all('SELECT fid ,fup,type,name FROM '.DB::table($this->_table)." WHERE ".DB::field('fid', $fids), array(), 'fid');
	}
	...
	public function update_commoncredits($fid) {
		return false; // zhangjh 2015-05-22 ����
		if(!intval($fid)) {
			return false;
		}
