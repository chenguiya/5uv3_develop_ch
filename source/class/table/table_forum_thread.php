...
class table_forum_thread extends discuz_table
{
	...
	//add by Daming 15/08/18
	public function my_fetch_all_by_tid($tids, $start = 0, $limit = 0, $field = '', $order = 'DESC', $tableid = 0) {
		$data = array();
		if(($data = $this->fetch_cache($tids)) === false || count($tids) != count($data)) {
			if(is_array($data) && !empty($data)) {
				$tids = array_diff($tids, array_keys($data));
			...
			if(!empty($tids)) {
				$parameter = array($this->get_table_name($tableid), $tids);
				$query = DB::query("SELECT * FROM %t WHERE tid IN(%n) ORDER BY $field $order".DB::limit($start, $limit), $parameter);
				while($value = DB::fetch($query)) {
					$data[$value['tid']] = $value;
					$this->store_cache($value['tid'], $value, $this->_cache_ttl);
				}
	...
	/**
	 * add by Daming get common thread list
	 * @param unknown $fids
	 * @param string $displayorder
	 * @param string $dateline
	 * @param string $recommends
	 * @param number $start
	 * @param number $limit
	 * @param string $order
	 * @param string $sort
	 ...
	 */	
	public function fetch_all_common_by_fid_displayorder($fids, $displayorder = null, $dateline = null, $recommends = null, $start = 0, $limit = 0, $order = 'dateline', $sort = 'DESC', $dglue = '>=') {
		$parameter = array($this->get_table_name());
		$wherearr = array();
		$fids = dintval($fids, true);
		if(!empty($fids)) {
...
// 		var_dump($wherearr);die;
	...
	public function fetch_all_by_fid_special_displayorder($fids, $displayorder = null, $special = 0, $start = 0, $limit = 0, $order = 'dateline', $sort = 'DESC', $dglue = '>=') {
		$parameter = array($this->get_table_name());
		$wherearr = array();
		$fids = dintval($fids, true);
		if(!empty($fids)) {
		...
		$ordersql = !empty($order) ? ' ORDER BY '.DB::order($order, $sort) : '';
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE special='.$special.' AND '.implode(' AND ', $wherearr) : ' WHERE special='.$special;
		return DB::fetch_all("SELECT * FROM %t $wheresql $ordersql ".DB::limit($start, $limit), $parameter, $this->_pk);
	}
	
	/**
	 * 获取版块/群组主题数，0 普通主题，4活动
	 * @param unknown $fid
	 * @param number $special
	 */
	public function count_thread_by_fid_type($fid, $special = 0) {
		return DB::result_first("SELECT COUNT(*) FROM %t t WHERE fid=%d AND special=%d", array($this->_table, $fid, $special));
	}	
}
