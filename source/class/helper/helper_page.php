...
class helper_page {
	...
	public static function multi($num, $perpage, $curpage, $mpurl, $maxpages = 0, $page = 7, $autogoto = FALSE, $simple = FALSE, $jsfunc = FALSE) {
		global $_G;
		$page=7;
		...
		if($num > $perpage) {
			...
			$_G['page_next'] = $to;
			$multipage = 
			($curpage > 1 && !$simple ? '<a href="'.(self::mpurl($mpurl, $pagevar, $curpage - 1)).$a_name.'" class="prev"'.$ajaxtarget.'>'.$lang['prev'].'</a>' : '').($curpage - $offset > 1 && $pages > $page ? '<a href="'.(self::mpurl($mpurl, $pagevar, 1)).$a_name.'" class="first"'.$ajaxtarget.'>1</a><b>'.$dot.'</b>' : '');
			for($i = $from; $i <= $to; $i++) {
			...
			$multipage .= ($to < $pages ? '<b>'.$dot.'</b><a href="'.(self::mpurl($mpurl, $pagevar, $pages)).$a_name.'" class="last"'.$ajaxtarget.'>'.$realpages.'</a>' : '').
			...
			$multipage = $multipage ? '<div class="pg common_pages">'.($shownum && !$simple ? '<em>&nbsp;'.$num.'&nbsp;</em>' : '').$multipage.'</div>' : '';
		}
		$maxpage = $realpages;
		return $multipage;
	}
