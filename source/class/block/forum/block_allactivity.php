...
class block_allactivity extends discuz_block {
	var $setting = array();
	
	function block_allactivity(){
		$this->setting = array(
				'tids' => array(
...
// 				),
	...
	function name() {
		return lang('blockclass', 'blockclass_forum_allactivity');
	}
	
	function blockclass() {
		return array('allactivity', lang('blockclass', 'blockclass_forum_allactivity'));
	}
...
// 		." AND t.isgroup='0'";
		...
		$query = DB::query("SELECT a.*, t.tid, t.fid, t.subject, t.authorid, t.author, t.isgroup$sqlfield
		...
		while($data = DB::fetch($query)) {
			$data['time'] = date('Y-m-d', $data['starttimefrom']);
			if($data['starttimeto']) {
				$data['time'] .= '&nbsp;至&nbsp;'.date('Y-m-d', $data['starttimeto']);
			}			
			if($style['getsummary']) {
			...
			$list[$data['tid']] = array(
					...
					'fields' => array(
							...
							'clublogo' => $forumfields[$data['fid']]['icon'],
			...
			if($data['starttimeto']) {
				$list[$data['tid']]['fields']['starttimeto'] = $data['starttimeto'];
			}
