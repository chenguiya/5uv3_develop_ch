...
class block_forum extends discuz_block {
	...
	function fields() {
		return array(
					...
					//add by Daming 2015/6/1
					'navmenu' => array('name' => '快捷导航', 'formtype' => 'text', 'datatype' => 'string'),
					//add end;
				);
	}
	...
	function getdata($style, $parameter) {
		...
		//edit by Daming 2015/6/1
// 		$ffadd1 = ", ff.icon, ff.description";
		$ffadd1 = ", ff.icon, ff.description, ff.threadtypes";
		//edit end;
		...
		while($data = DB::fetch($query)) {
			...
			//add by Daming 2015/6/1
			$typetitle = array('新闻' => 'news',	 '图片' => 'pic', '视频' => 'video');
			$typecss = array('新闻' => 'news', '图片' => 'photo', '视频' => 'video');
			if (!empty($data['threadtypes'])) {				
				$threadtypes = dunserialize($data['threadtypes']);				
				$data['navmenu'] = '<a class="ico_topic" href="forum.php?mod=forumdisplay&fid='.$data['fid'].'&flag=index"><i>&nbsp;</i>话题</a>';
				foreach ($threadtypes['types'] as $key => $val) {
					$data['navmenu'] .= '<a class="ico_'.$typecss[$val].'" href="forum.php?mod=forumdisplay&fid='.$data['fid'].'&tpl=channel_'.$typetitle[$val].'&typeid='.$key.'&filter=typeid"><i>&nbsp;</i>'.$val.'</a>';
				}
				$data['navmenu'] .= '<a class="ico_fans" href="plugin.php?id=fansclub:forumfansclub&fid='.$data['fid'].'"><i>&nbsp;</i>球迷会</a>';
			} else {
				$data['navmenu'] = '';
			}
			//add end;
