...
class block_thread extends discuz_block {
	...
	function fields() {
		return array(
					...
					'sheartimes' => array('name' => '分享次数', 'formtype' => 'text', 'datatype' => 'int'),
					'recommend_add' => array('name' => '支持数', 'formtype' => 'text', 'datatype' => 'int')
				);
	}
	...
	function getdata($style, $parameter) {
		...
		while($data = DB::fetch($query)) {
			// 2015-06-30 zhangjh 修改 sharetimes/recommend_add/replies 读数据库实时数据
			C::t('forum_thread')->clear_cache(intval($data['tid'])); // 清缓存
			$sql2 = DB::query("SELECT a.recommend_add, a.replies, a.sharetimes
					FROM `".DB::table('forum_thread')."` a
					WHERE a.tid = ".intval($data['tid']));
			$data2 = DB::fetch($sql2);
			...
			$list[$data['tid']] = array(
				...
				'fields' => array(
					...
					//add by Daming 2015/6/1
					'sheartimes' => $data2['sharetimes'],
					'recommend_add' => $data2['recommend_add'],
					//add end;
