...
switch($frommodcp) {
	...
	default:
		if(in_array('delete', $operations) || in_array('move', $operations) && !strpos($_SERVER['HTTP_REFERER'], 'search.php?mod=forum')) {
            ...
            {
                $_G['referer'] = 'forum.php?mod=group&fid='.$_G['fid'].(!empty($_GET['listextra']) ? '&'.rawurldecode($_GET['listextra']) : '').'&mobile='.$_GET['mobile'];
            }
            else
            {
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				//edit by Daming 2015/9/23 去掉限制，以实现可以设置特殊置顶（为球迷会规则贴修改）
// 				if($sticklevel < 0 || $sticklevel > 5 || $sticklevel > $_G['group']['allowstickthread']) {
