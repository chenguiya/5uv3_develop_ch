...
if($_GET['view'] == 'me') {
	...
	if($viewtype == 'thread' && $allowview) {
		...
		$ordersql = 't.tid DESC';
		$subjectarr = array(); 
		$threadarr = C::t('forum_thread')->fetch_all_by_authorid($_GET['uid'], $start, $perpage);

		if(count($threadarr) != 0){
			$subjectarrlen = 1;
			foreach($threadarr as $key=>$value) {
				if($value['displayorder'] > -1){
					$subjectarr[$key]['displayorder'] = $value['displayorder'];
					$subjectarr[$key]['replies'] = $value['replies'];
					$subjectarr[$key]['subject'] = $value['subject'];
					if(time()-$value['dateline'] < 24*60*60){
						if(intval((time()-$value['dateline'])/60) < 1){
							$subjectarr[$key]['dateline'] = '1分钟前';
						}elseif(intval((time()-$value['dateline'])/60) > 1 && intval((time()-$value['dateline'])/60) < 60){
							$subjectarr[$key]['dateline'] = intval((time()-$value['dateline'])/60).'分钟前';
						}else{
							$subjectarr[$key]['dateline'] = intval((time()-$value['dateline'])/(60*60)).'小时前';
						}
					}else{
						$subjectarr[$key]['dateline'] = date('Y年m月d日 H:i:s',$value['dateline']);
					}
				}
