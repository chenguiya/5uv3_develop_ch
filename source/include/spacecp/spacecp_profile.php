...
if(submitcheck('profilesubmit')) {
	...
	foreach($_POST as $key => $value) {
		...
		if(in_array($field['formtype'], array('text', 'textarea')) || in_array($key, $forumfield)) {
			...
			if($censor->modbanned() || $censor->modmoderated()) {
                if($bln_mobile)
                {
                    showmessage(lang('spacecp', 'profile_censor'), 'home.php?mod=spacecp&ac=profile&mobile=2');
                }
                else
                {
		...
		if(empty($field)) {
            ...
            if($bln_mobile)
            {
                showmessage($key.'有误', 'home.php?mod=spacecp&ac=profile&mobile=2');
            }
            else
            {
	...
	if($_FILES) {
		...
		foreach($_FILES as $key => $file) {
			...
			if(!profile_check($key, $value, $space)) {
                ...
                if($bln_mobile)
                {
                    showmessage(lang('spacecp', 'filesize_lessthan').$field['size'].'KB', 'home.php?mod=spacecp&ac=profile&mobile=2');
                }
                else
                {
    ...
    // zhangjh 2015-09-01 如果是手机端，修改返回提示
    if($bln_mobile)
    {
        showmessage('profile_succeed', 'home.php?mod=spacecp&ac=profile&mobile=2');
    }
    else
    {
	...
	if($authstr) {
        ...
        if($bln_mobile)
        {
            showmessage('密码修改成功', 'home.php?mod=spacecp&ac=profile&op=password&mobile=2');
        }
        else
        {
...
if($operation == 'password') {
	...
	if(in_array($operation, array('base', 'contact', 'edu', 'work', 'info'))) {
        ...
        // zhangjh 2015-09-01 如果是手机端，一并显示bas，contact和info
        if($bln_mobile)
        {
            $allowitems = array_merge($profilegroup['base']['field'], $profilegroup['contact']['field'], $profilegroup['info']['field']);
        }
