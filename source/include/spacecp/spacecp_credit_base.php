...
if($_GET['op'] == 'base') {
	...
	if(submitcheck('addfundssubmit')) {
		...
		if($apitype == 'card') {
			...
			$requesturl = credit_payurl($price, $orderid, $_GET['bank_type'], 'JF'); // zhangjh 2015-09-14 加订单号前缀
            ...
            // zhangjh 2015-09-17 多增加记录 
            // 写 pre_plugin_ucharge_log 表，自定义的
			$arr_data = array();
			$arr_data['orderid'] = $orderid;
			$arr_data['status'] = '1';
			$arr_data['log_time'] = $_G['timestamp'];
			$arr_data['amount'] = $amount;
			$arr_data['price'] = $price;
			$arr_data['uid'] = $_G['uid'];
			$arr_data['fid'] = '0';
			$arr_data['charge_type'] = '0';
			$arr_data['username'] = $_G['member']['username'];
			$arr_data['email'] = $_G['member']['email'];
			$arr_data['ip'] = $_G['clientip'];
			$arr_data['subject'] = '积分充值';
			$arr_data['body'] = '积分充值';
			$arr_data['seller_email'] = $_G['setting']['ec_account'];
			$arr_data['notify_url'] = $_G['siteurl'].'api/trade/notify_credit.php';
			$arr_data['return_url'] = $_G['siteurl'].'api/trade/notify_credit.php';
			$arr_data['show_url'] = $_G['siteurl'];
            $arr_data['bill_type'] = '1'; // 2015-09-17 多增加一类型 1:积分充值 2:VIP充值
