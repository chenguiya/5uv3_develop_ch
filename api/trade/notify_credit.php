...
if($notifydata['validator']) {
	...
	if($order && floatval($postprice) == floatval($order['price']) && ($apitype == 'tenpay' || strtolower($_G['setting']['ec_account']) == strtolower($_REQUEST['seller_email']))) {
		...
		if($order['status'] == 1) {
            ...
            // 更新附表 by zhangjh 2015-09-17
			C::t('#ucharge#plugin_ucharge_log')->update($orderid, 
				array('status' => '2', 
					  'confirm_time' => $_G['timestamp'], 
					  'api_type' => $apitype, 
					  'trade_no' => $notifydata[trade_no]));
                      
			C::t('forum_order')->update($orderid, array('status' => '2', 
			...
			// C::t('forum_order')->delete_by_submitdate($_G['timestamp']-60*86400); // 不删除之前的记录 by zhangjh 2015-09-16
