...
class usermodel {
	...
	function check_username($username) {
        ...
        if($len > 15 || $len < 1 || preg_match("/\s+|^c:\\con\\con|[%,\*\"\s\<\>\&]|$guestexp/is", $username)) { // 2015-09-28 zhangjh 改成1个长度
			return FALSE;
		} else {
			return TRUE;
		}
    ...
    {
        $query = $this->db->query("SELECT * FROM ".UC_DBTABLEPRE."memberfields WHERE openid='".$openid."'");
