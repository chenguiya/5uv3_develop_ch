...
.postalbum { background-color:rgba(0,0,0,.8); display:none; height:100%; overflow:hidden; padding:0 0 1px 0; position:absolute; top:0; width:100%; z-index:80; }
...
.postalbum_h_picnum { position:absolute; right:10px; top:0px; height:25px;  z-index:90; color:white; }
...
</style><?php $curaidkey = 0;?><?php $count = count($imglist['aid']);?><?php if($_GET['aid']) { if(is_array($imglist['aid'])) foreach($imglist['aid'] as $k => $aid) { if($_GET['aid'] == $aid) { $curaidkey = $k;break;?><?php } } } ?>
<div class="postalbum">
<div class="postalbum_h">
<a href="forum.php?mod=viewthread&amp;tid=<?php echo $_G['tid'];?>&amp;page=<?php echo $_G['page'];?>" class="postalbum_h_back"><i class="iconfont icon-back"></i></a>
<span class="postalbum_h_picnum"> <span id="curpic"><?php echo $curaidkey + 1?></span> / <?php echo $count;?> </span>
</div>
<ul class="postalbum_c"><?php if(is_array($imglist['url'])) foreach($imglist['url'] as $key => $imgurl) { ?><li class="postalbum_u" id="u_<?php echo $key;?>"><?php $imgurl = getforumimg($imglist[aid][$key], 0, 2000, 550, 'fixnone');?><img class="postalbum_i" load="0" id="img_<?php echo $key;?>" <?php if($curaidkey == $key) { ?>src="<?php echo $imgurl;?>"<?php } ?> zsrc="<?php echo $imgurl;?>" orig="<?php echo $imglist['url'][$key];?>"/>
</li>
<?php } ?>
</ul>
