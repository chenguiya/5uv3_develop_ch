<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); $_G[forum_thread][special] = 0;?><!-- main postlist start --><?php $needhiddenreply = ($hiddenreplies && $_G['uid'] != $post['authorid'] && $_G['uid'] != $_G['forum_thread']['authorid'] && !$post['first'] && !$_G['forum']['ismoderator']);?>   <div class="chtwo cl">
        <div class="viewhd">
             <div class="vi_avatar_img"><a href="home.php?mod=space&amp;uid=<?php echo $post['authorid'];?>&amp;do=profile&amp;mobile=2" style="border:none;"><img src="<?php echo avatar($post[authorid], middle, true);?>" /></a></div>
             <div class="two_wrap">
                  <div class="authi_tw">
                        <strong>
                        ...
                        <?php } elseif($post['number'] == -1) { ?>
                            推荐
                        <?php } else { ?>
                            <?php if(!empty($postno[$post['number']])) { ?><?php echo $postno[$post['number']];?><?php } else { ?><?php echo $post['number'];?><?php echo $postno['0'];?><?php } ?>
                        <?php } ?>
                        </strong>
                        <b>
                    <?php if($post['authorid'] && $post['username'] && !$post['anonymous']) { ?>
                        <a href="home.php?mod=space&amp;uid=<?php echo $post['authorid'];?>&amp;do=profile&amp;mobile=2" class="blue"><?php echo $post['author'];?></a></b>
                    ...
                    <?php } else { ?>
                        ...
                        <?php if($_G['forum']['ismoderator']) { ?><a href="home.php?mod=space&amp;uid=<?php echo $post['authorid'];?>&amp;do=profile&amp;mobile=2" target="_blank">匿名</a><?php } else { ?>匿名<?php } ?>
                        <?php } else { ?>
                        <?php echo $post['author'];?> <em>该用户已被删除</em>
                        <?php } ?>
                    <?php } ?>
                    <b class="grey rela"><?php echo $post['dateline'];?></b>
                  </div>
