...
<?php if(($_GET['optgroup'] == 3 && $operation == 'delete') || ($_GET['optgroup'] == 4 && $operation == '') || ($_GET['optgroup'] == 3 && $operation == 'stick')) { ?>
...
<dd><input type="submit" class="formdialog button2" name="modsubmit" id="modsubmit"  value="确定"><a href="javascript:;" onclick="popup.close();">取消</a></dd>
                <?php } else { ?>
                    <dt>您没有删除此主题权限</dt>
                <?php } ?>
            <?php } elseif($operation == 'stick') { ?>
            	<?php if($_G['group']['allowdelpost']) { ?>
                    <input name="operations[]" type="hidden" value="stick"/>                    
                    <input name="sticklevel" type="hidden" value="<?php echo $_GET['sticklevel'];?>">
                    <?php if($_GET['sticklevel'] == 0) { ?>                    
                    <dt>你确认要取消置顶这篇帖子吗？</dt>
                    <?php } else { ?>
                    <dt>你确认要置顶这篇帖子吗？</dt>
                    <?php } ?>
